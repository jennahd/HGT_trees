...
This script takes in an input protein phylogeny and outputs a labelled
pdf with taxonomy coloured, and focal taxa labelled.
"""
#General modules
import os
import argparse
...
parser.add_argument('-info', '--info', required=True, help = 'File with \
	descritive information about each cluster.')

parser.add_argument('-phyla', '--phyla', required=True, help = 'File with \
	all phyla found across trees of interest.')

parser.add_argument('-domains', '--domains', required=True, help = 'File with \
	pfam interproscan domains output.')

parser.add_argument('-faa', '--faa', required=True, help = 'File with \
	the unaligned faa sequenced used to infer the tree.')

parser.add_argument('-focal_taxonomy', '--focal_taxonomy', required=True, help = 'Taxonomic \
	identifier of group of interest.')

parser.add_argument('-out', '--out', required=True, help = 'Name of output \
	pdf file for labelled tree.')
...
#params
focal_taxonomy = args.focal_taxonomy
cluster_name = tree.split("/")[-1].split(".")[0]
num = tree.split("/")[-1].split(".")[1]
...
try:
	...
	if len(outgroup_taxa) == 0:
		t = root_tree_no_out(t, focal_node_dict["0"])
	else:

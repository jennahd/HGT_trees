...
This script takes in an input protein phylogeny and determines the putative
taxonomy of the donor of the protein to a given focal group of interest.
"""
#General modules
import os
import argparse
...
parser = argparse.ArgumentParser(prog='HGT_source.py',
	formatter_class=argparse.ArgumentDefaultsHelpFormatter,
	description='Find HGT donor of protein to focal group of interest.')

parser.add_argument('-tree', '--tree', required=True, help = 'File with \
	the tree output.')

parser.add_argument('-map', '--map', required=True, help = 'File with \
	species IDs, names, and focal taxa.')

parser.add_argument('-tax', '--tax', required=True, help = 'Taxonomic \
	identifier of group of interest.')

parser.add_argument('-out', '--out', required=True, help = 'Name of output \
	tsv file with sister and nested taxonomy at different cutoffs.')

parser.add_argument('-cluster', '--cluster', required=True, help = 'Cluster ID.')

args = parser.parse_args()

if not os.path.exists(args.tree):
	raise IOError("The tree file does not exist")
...
def set_far_node(tree, focal_node_dict):
	far_node = None
	far_node_dist = 0
	if focal_node_dict["0"] != tree:
		tree.set_outgroup(focal_node_dict["0"])
		for node in tree.traverse():
			dist = tree.get_distance(focal_node_dict["0"], node, topology_only=True)
			if node not in focal_node_dict["0"] and node != tree and dist > far_node_dist and len(node) >= 2:
				far_node = node
				far_node_dist = dist
	if far_node != None:
...
def find_sister_group(tree, focal_node):
	...
	try:
		...
		while sister_node.support < 80 and sister_node != tree:
...
#params
cluster_name = args.cluster.split(".")[0]
cluster_num = args.cluster.split(".")[1]
focal_taxonomy = args.tax
...
focal_node_dict = get_focal_node_dict(tree, focal_taxa_dict)
...
if len(outgroup_list) > 0:
	...
	tree = set_far_node(tree, focal_node_dict)
...
with open(out, "w") as f:
	for num, focal_node in focal_node_dict_up.items():
		...
		for cutoff in cutoffs:
			...
			print(cutoff)
			print(sister_majority_tax)
			print(nested_majority_tax)
			f.write("{0}\t{1}\t{2}\t{3}\t{4}\n".format(cluster_name, cluster_num, cutoff, sister_majority_tax, nested_majority_tax))
